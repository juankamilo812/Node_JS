...
app.post('/details/InsertarProductos', async (req, res) => {
    let respuesta = [];
    //for (let producto of req.fields.id_producto) {
    for (let i = 1; i <= req.fields.id_producto.length; i++) {
        let details = new Details({
            id_pedido: req.fields.id_pedido,
            //id_producto: producto
            id_producto: req.fields.id_producto[i]
        });
        respuesta.push(await details.InsertarProductos());
    }
    res.send(respuesta);
});
